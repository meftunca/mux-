...
enum Code {
  ...
  SwitchingProtocols =
      101, //!< Indicates that the server understands and is willing to comply
           ...
           //!< connection.
  Processing =
      102, //!< Is an interim response used to inform the client that the server
  ...
  NoContent = 204, //!< Indicates that the server has successfully fulfilled the
                   //!< request and that there is no additional content to send
  ...
  AlreadyReported =
      208, //!< Used inside a DAV:propstat response element to avoid enumerating
  ...
  MultipleChoices =
      300, //!< Indicates that the target resource has more than one
           ...
           //!< information about the alternatives is being provided so that the
           //!< user (or user agent) can select a preferred representation by
  ...
  Found = 302, //!< Indicates that the target resource resides temporarily under
               //!< a different URI.
  ...
  UseProxy =
      305, //!< \deprecated \parblock Due to security concerns regarding in-band
           //!< configuration of a proxy. \endparblock The requested resource
  ...
  PermanentRedirect =
      308, //!< The target resource has been assigned a new permanent URI and
           //!< any future references to this resource outght to use one of the
           //!< enclosed URIs. [...] This status code is similar to 301 Moved
           //!< Permanently (Section 7.3.2 of rfc7231), except that it does not
           //!< allow rewriting the request method from POST to GET.

  /*####### 4xx - Client Error #######*/
  /* Indicates that the client seems to have erred.
   */
  BadRequest =
      400, //!< Indicates that the server cannot or will not process the request
           //!< because the received syntax is invalid, nonsensical, or exceeds
           //!< some limitation on what the server is willing to process.
  Unauthorized =
      401, //!< Indicates that the request has not been applied because it lacks
  ...
  NotAcceptable =
      406, //!< Indicates that the target resource does not have a current
           ...
           //!< according to the proactive negotiation header fields received in
           //!< the request, and the server is unwilling to supply a default
           //!< representation.
  ProxyAuthenticationRequired =
      407, //!< Is similar to 401 (Unauthorized), but indicates that the client
  ...
  Conflict = 409, //!< Indicates that the request could not be completed due to
  ...
  ContentTooLarge = 413, //!< Indicates that the server is refusing to process a
                         //!< request because the request payload is larger than
                         //!< the server is willing or able to process.
  PayloadTooLarge =
      413,          //!< Alias for ContentTooLarge for backward compatibility.
  ...
  UnsupportedMediaType =
      415, //!< Indicates that the origin server is refusing to service the
           //!< request because the payload is in a format not supported by the
           //!< target resource for this method.
  RangeNotSatisfiable =
      416, //!< Indicates that none of the ranges in the request's Range header
           //!< field overlap the current extent of the selected resource or
           //!< that the set of ranges requested has been rejected due to
           //!< invalid ranges or an excessive request of small or overlapping
           //!< ranges.
  ...
  MisdirectedRequest = 421, //!< Indicates that the request was directed at a
                            //!< server that is unable or unwilling to produce
                            //!< an authoritative response for the target URI.
  UnprocessableContent =
      422, //!< Means the server understands the content type of the request
           ...
           //!< inappropriate), and the syntax of the request entity is correct
           //!< (thus a 400 (Bad Request) status code is inappropriate) but was
           //!< unable to process the contained instructions.
  UnprocessableEntity =
      422, //!< Alias for UnprocessableContent for backward compatibility.
  Locked =
  ...
  FailedDependency =
      424, //!< Means that the method could not be performed on the resource
           ...
           //!< action failed.
  TooEarly = 425, //!< Indicates that the server is unwilling to risk processing
  ...
  PreconditionRequired = 428, //!< Indicates that the origin server requires the
  ...
  UnavailableForLegalReasons =
      451, //!< This status code indicates that the server is denying access to
           //!< the resource in response to a legal demand.
  ...
  InsufficientStorage =
      507, //!< Means the method could not be performed on the resource because
  ...
  NotExtended = 510, //!< \deprecated \parblock Obsoleted as the experiment has
                     //!< ended and there is no evidence of widespread use.
                     //!< \endparblock The policy for accessing the resource has
                     //!< not been met in the request. [RFC 2774]
