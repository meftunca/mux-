...
struct SockerReaderRoutinePromise;
template <>
struct std::coroutine_traits<SockerReaderRoutinePromise> {
  using promise_type = SockerReaderRoutinePromise;
};
struct SockerReaderRoutinePromise {
  std::string value;
  std::suspend_always initial_suspend() { return {}; }
  std::suspend_always final_suspend() { return {}; }
  std::string get_return_object() { return value; }
  void return_value(std::string value) { this->value = value; }
  void unhandled_exception() { std::terminate(); }
};
class SocketReader {
 public:
  SocketReader(int socket) : m_socket_(socket) {}
  ~SocketReader() { close(m_socket_); }
  std::coroutine_handle<SockerReaderRoutinePromise> operator()() {
    return std::coroutine_handle<SockerReaderRoutinePromise>::from_promise(
        m_promise_);
  }

  void operator()(std::coroutine_handle<SockerReaderRoutinePromise> handle) {
    std::string buffer(1024, '\0');
    int bytesRead = read(m_socket_, buffer.data(),
                         buffer.size() - 1);  // 1 byte boşluk bırak
    if (bytesRead > 0) {
      buffer.resize(bytesRead);
      m_promise_.return_value(buffer);
    } else if (bytesRead == 0) {
      m_promise_.return_value("");  // Boş string döndür
    } else {
 ...
 private:
  int m_socket_;
  SockerReaderRoutinePromise m_promise_;
};
/*// Example
 SocketReader reader(clientSocket);
std::string buffer = reader().promise().value;
if (buffer.empty()) {
  close(clientSocket);
  return;
}
...
class Socket {
 public:
  explicit Socket(int port = 3732) : m_serverSocket(-1), m_port(port) {}
  ~Socket() {
    closeServerSocket();
  ...
  void handleConnection(int clientSocket, int kqOrEpollfd = -1) {
    ...
    auto handle = reader();
    reader(handle);
    std::string buffer = handle.promise().value;
    handle.destroy();
    if (buffer.empty()) {
    ...
    auto resp = createHttpResponse(buffer);
    std::cout << "Response: " << resp << std::endl;
    sendData(clientSocket, resp.c_str(), resp.size());
    // char buffer[1024] = {0};
    // int bytesRead =
  ...
  void handleAndResumeConnections() {
    ...
    while (true) {
      ...
      int activity = kevent(kq, NULL, 0, events, 32, NULL);
#else
      epoll_event events[32];
      int activity = epoll_wait(epollfd, events, 32, -1);
#endif
      ...
      for (int i = 0; i < activity; i++) {
        ...
        if (events[i].filter == EVFILT_READ) {
          if (events[i].ident == static_cast<uintptr_t>(m_serverSocket)) {
            // Yeni bağlantı kabul etme
            int new_socket = acceptConnection(m_serverSocket);
 ...
 private:
  ...
  //     .tv_sec = 1,
  //     .tv_usec = 0};  // Select fonksiyonunun timeout süresi
  // ...
};
/*
Example usage:
